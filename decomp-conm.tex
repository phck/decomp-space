...
\begin{exercise}
Show that every partial monoid is 2-Segal using only Definition\nobreakspace \ref {def 2-segal}.
\end{exercise}
...
% X_n \rar[swap]{d_1^{n-1}} & X_1
% \end{tikzcd} \]
% instead. Formally, one can apply Proposition\nobreakspace \ref {prop fewer squares} to $X^\op$.
...
\end{align*}
One checks that $(\udec X)^\op = \ldec(X^\op)$, so that applying Theorem\nobreakspace \ref {thm upper path} to $X^\op$ we obtain the following.

\begin{corollary}
A simplicial space $X$ is lower 2-Segal if and only if its lower d\'ecalage $\ldec X$ is Segal. \qed
\end{corollary}

Using Theorem\nobreakspace \ref {thm unitality}, we conclude the original form of the path space criterion.
...
When $X$ is a decomposition space, it turns out that the maps $d_\top \colon \udec X \to X$ and $d_\bot \colon \ldec X \to X$ are culf (in the sense of Definition\nobreakspace \ref {def culf}); see \cite[Theorem~4.10]{GKT1}.
...
The proof presented here utilizes the exact same techniques that we have been using since Section\nobreakspace \ref {sec 2-Segl decomp}.
This proof relies on the path space criterion (Corollary\nobreakspace \ref {cor path space}), which is appropriate as there is a close connection between the edgewise subdivision and the upper/lower d\'ecalage pair.\phnote{Additionally, this theorem can be interpreted in terms of the double-categorical perspective from \cite{BOORS:2SSWC}.}
...
If our original map is culf (Definition\nobreakspace \ref {def culf}), then the map $A_{\ell_1, \dots, \ell_k} \to A_m$ (with $m = \sum \ell_i$) is an isomorphism.

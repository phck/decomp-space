...
The aim of this paper is to give a gentle introduction to decomposition spaces, similar to Stern's introduction to 2-Segal spaces from this volume \cite{Stern:BIRS}.
The theory of decomposition spaces begs us to reduce as much as possible to a calculus of pullback squares, and we try to follow this as much as possible whenever we can.
All told, the prerequisites for this work are mild, and similar to \cite{Stern:BIRS}: some elementary category theory, and some comfortability with simplicial sets (but see \cref{sec ways to read}).
...
The first of these is simple: decomposition spaces are meant to be a generalization of Segal spaces, and we establish this is indeed the case.
Additionally, Segal spaces are characterized among the decomposition spaces by means of a single square.
...
This includes notions of functoriality via suitable classes of maps, which are discussed in \cite{CooperYoung:BIRS,GKT:DSC}.
...
\item All statements before \cref{sec free} also hold, using the same arguments, for simplicial objects in a 1-category $C$.\footnote{Typically, one would require that $C$ appearing in \eqref{item gen cat} and \eqref{item gen inf cat} have finite limits.
...
Special thanks go to Matt Feller, Joachim Kock, Justin Lynd, Felix Naß, and Walker Stern.

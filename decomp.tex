...
\usepackage[unicode,
  ...
  urlcolor=MediumOrchid]{hyperref}
...
\begin{comment}
We assume some familiarity with simplicial techniques, as used for instance in \cite[Chapter 8]{Weibel:IHA}. And with categories.

Higher Segal connections (pasting law etc.)??
\end{comment}


\begin{comment}
\begin{convention}[Spaces versus sets]\label{conv spaces sets}
In following the original sources, we will mostly speak of \emph{simplicial spaces}, by which we mean objects in the $(\infty,1)$-category $\sspace \coloneqq \fun(\Delta^\op, \spaces)$ of space-valued presheaves on $\Delta$.
But everything holds equally well for simplicial \emph{sets}, and we encourage readers to instead work with the 1-category $\sset \coloneqq \fun(\Delta^\op, \set)$.
\end{convention}
\end{comment}

\section{Introduction}

Decomposition spaces were introduced by Imma G\'alvez-Carillo, Joachim Kock, and Andrew Tonks in a long preprint in 2014, which now comprises six papers \cite{GKT1,GKT2,GKT3,GKT:DSC,GKT:DSRS,GKT:HLA}.
...
% \cite{Carlier:HSMDS} and \cite{Carlier:IBMIRF} and \cite{CarlierKock:AMDS} and \cite{CebrianForero:DHSDS} and \cite{Forero:GCKTCLDDS}


Decomposition spaces are the same thing as the 2-Segal spaces of Dyckerhoff and Kapranov \cite{DyckerhoffKapranov:HSS}. 
...
We will give an account of this result in \cref{sec decomp vs 2-segal}, following \cite{GKT1,Feller_et_al:E2SSU}.

% Write about the path space criterion
...
% With one exception (\cref{ex forests}), 
...
\item All statements before \cref{sec free} also hold, using the same arguments, for simplicial objects in a 1-category $C$ having pullbacks and a terminal object.\footnote{Note that the assumption of existence of finite limits in \eqref{item gen cat} and \eqref{item gen inf cat} is only used to assert that certain fiber products exist, as in \cref{def Segal}.
This assumption can be dropped, by instead asking that certain diagrams arising from simplicial objects \emph{are} limit diagrams.}
...
% These include the model structure for 2-Segal spaces from \cite{DyckerhoffKapranov:HSS} and the quasi-2-Segal sets of Feller \cite{Feller:Q2SS}
\phnote{Cite a few such sources.}

\subsection*{Acknowledgements}
This paper originated from the author's talks at the workshop ``Higher Segal Spaces and their Applications to Algebraic K-Theory, Hall Algebras, and Combinatorics'' held at the Banff International Research Station (BIRS) in January 2024.
The author thanks the workshop organizers (Julie Bergner, Joachim Kock, Maru Sarazola) for cultivating a vibrant and stimulating atmosphere and giving me the opportunity to speak, and BIRS for providing an ideal venue for the event.
Thanks also to Mark Penney for his work (together with Julie Bergner) in organizing a predecessor workshop planned for 2020 at Casa Matemática Oaxaca, which was unfortunately cancelled due to the COVID-19 pandemic.
The author is grateful to the other participants for their contributions and discussions.
Special thanks go to Matt Feller for discussion concerning edgewise subdivision (\cref{sec edgewise}).
...
For each $i = 0, 1, \dots, n$, there is a \emph{coface map} $\delta^i \colon [n-1] \to [n]$ which is injective and does not have $i$ in its image, and a \emph{codegeneracy map} $\sigma^i \colon [n+1] \to [n]$ which is surjective with the preimage of $i$ having cardinality two. %\footnote{Strictly speaking, we should tag these with an $n$ as well.}
In formulas:
\begin{align*}
...
A fundamental example of \cite{GKT1} %, which will reappear in \cite{Tonks:BIRS}, 
...
% \[ \begin{tikzcd}
% \includegraphics[scale=0.2]{tree_nw.pdf} \rar[mapsto,"d_2"] & \includegraphics[scale=0.2]{tree_ne.pdf} 
% \end{tikzcd} \]
...
\end{tikzpicture}
\end{center}
% Morphisms are more delicate, and it turns out that the squares from \eqref{eq upper and lower 2-Segal} are only \emph{homotopy} pullbacks of groupoids.
\end{example}
...
In a similar manner, the $\catpar$-enriched categories from the preceding exercise correspond to categories with zero morphisms. % \cite[\S1.7]{Pareigis:CF}.
\end{remark}
...
Suppose $X$ is an upper %(resp.\ lower) 
2-Segal space.
...
\]
with $DS = \id$.
%See \cite{Feller_et_al:E2SSU} for details. 
...
\end{exercise}
% \footnote{Additional equivalent conditions may be found in \cite[Lemma 2.10]{GKT1}.}

% As in \cref{conv pullback}, the pullbacks appearing in \eqref{item usual Segal} are homotopy pullbacks.
% Note that $X$ is Segal if and only if $X^\op$ is Segal.
...
\end{corollary}\phnote{I haven't mentioned that $\udec X \to X$ and $\ldec X \to X$ turn out to be culf.}
...
% Lastly, we conclude with the following theorem from \cite{KockSpivak:DSST}

% \begin{theorem}[Kock \& Spivak]
...
\begin{equation}%\label{diag outer face} 
\begin{tikzcd}
